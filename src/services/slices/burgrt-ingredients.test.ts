import reducer, {
	...
	initialState, setCurrentBurgerIngredientAction
...
describe('burger-constructor reducer', () => {
	...
	it("should handle clearCartAction", () => {
		const state = { ...initialState, burgerIngredients: MOCK_INGREDIENTS }
		expect(reducer(state, setCurrentBurgerIngredientAction('60d3b41abdacab0026a733c6'))).toEqual({
			...initialState,
			burgerIngredients: MOCK_INGREDIENTS,
			currentBurgerIngredient: MOCK_INGREDIENTS.find(itemt => itemt._id === '60d3b41abdacab0026a733c6')
		});
	});
	it("should handle clearBurgerIngredientAction", () => {
		const state = { ...initialState, burgerIngredients: MOCK_INGREDIENTS }
		expect(reducer(state, clearBurgerIngredientAction())).toEqual({
			...initialState,
			burgerIngredients: MOCK_INGREDIENTS,
	...
	describe('should handle fetchBurgerIngredients thunk', () => {
		it('should handle fetchBurgerIngredients.pending', () => {
			const action = { type: fetchBurgerIngredients.pending };
		...
		it('should handle fetchBurgerIngredients.fulfilled', () => {
			...
			const action = {
				type: fetchBurgerIngredients.fulfilled,
				payload: mock
			};
			expect(reducer(initialState, action)).toEqual({
				...initialState,
				burgerIngredients: mock.data,
			})
		});
		it('should handle fetchBurgerIngredients.rejected', () => {
			const action = { type: fetchBurgerIngredients.rejected };
			expect(reducer(initialState, action)).toEqual({
				...initialState,
